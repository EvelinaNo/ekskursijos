...
const CreateExcursion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:1000/api/triptrack/excursions', {
      ...
      for (const date_time of formData.date_times) {
        await axios.post(`http://localhost:1000/api/triptrack/schedule/${excursionId}/addTimeSlot`, {
          excursion_id: excursionId,
          date_time,
        });
      }

      const newExcursion = response.data;

      const storedExcursions = JSON.parse(sessionStorage.getItem('excursions')) || [];

      const updatedExcursions = [...storedExcursions, newExcursion];

      sessionStorage.setItem('excursions', JSON.stringify(updatedExcursions));

      navigate('/excursions');
    } catch (error) {
      setErrors({ api: 'Error creating excursion: ' + error.message });
  ...
  return (
    <RegistrationContainer>
      <FormTitle>Create new excursion</FormTitle>
